...
void delayForISR(unsigned long ms)
{
...
*  False otherwise
*/
boolean SWPACKET::send(
	unsigned int	NumberOfRetries,
	unsigned long	DelayBeforeSending,
	unsigned long	DelayBetweenSending)
{
	...
	delayForISR(DelayBeforeSending);

	for (i=0; i<NumberOfRetries; i++)
	{
		if((res = panstamp.cc1101.sendData(&packet)) == true)
		{
			break;
		}
		delayForISR(DelayBetweenSending);
	}
