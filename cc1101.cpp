...
CC1101::CC1101(void)
{
  paTableByte = PA_7dBm;			// Default = +7dBm
}
...
void CC1101::wakeUp(void)
{
  ...
  cc1101_Deselect();                // De-select CC1101
}
...
void CC1101::setSyncWord(uint8_t syncH, uint8_t syncL, bool save) 
{
  if ((syncWord[0] != syncH) || (syncWord[1] != syncL))
  {
    ...
    if (save)
    {
      EEPROM.put(EEPROM_SYNC_WORD, syncH);
      EEPROM.put(EEPROM_SYNC_WORD + 1, syncL);
    }
  }
}
...
void CC1101::setDevAddress(byte addr, bool save) 
{
  if (devAddress != addr)
  {
    ...
    // Save in EEPROM
    if (save)
      EEPROM.put(EEPROM_DEVICE_ADDR, addr);  
  }
}
...
void CC1101::setChannel(byte chnl, bool save) 
{
  if (channel != chnl)
  {
    ...
    // Save in EEPROM
    if (save)
      EEPROM.put(EEPROM_FREQ_CHANNEL, chnl);
  }
}
...
void CC1101::setRegsFromEeprom(void)
{
  ...
  EEPROM.get(EEPROM_FREQ_CHANNEL, bVal);
  // Set RF channel
  if (bVal < NUMBER_OF_FCHANNELS )
    setChannel(bVal, false);
  // Read Sync word from EEPROM
  EEPROM.get(EEPROM_SYNC_WORD,		arrV[0]);
  EEPROM.get(EEPROM_SYNC_WORD + 1, arrV[1]);
  ...
  EEPROM.get(EEPROM_DEVICE_ADDR, bVal);
  // Set device address
  if (bVal > 0)
...
boolean CC1101::sendData(CCPACKET * packet)
{
  ...
  setIdleState();       // Enter IDLE state
  flushTxFifo();        // Flush Tx FIFO
  setRxState();			// Enter back into RX state
