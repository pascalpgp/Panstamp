...
#define SWAP_MODEM_ADDR					0xFD									// SWAP modem address

#define SWAP_NUMBER_OF_RETRIES			(unsigned int)(8)																 // Number of transmission retries

#define SWAP_DELAY_BEFORE_SENDING		(unsigned long)(  25UL * (unsigned long)    panstamp.cc1101.devAddress)			 // Delay before sending
#define SWAP_DELAY_BEFORE_REPEATING		(unsigned long)( 250UL * (unsigned long) (((panstamp.cc1101.devAddress-1)%8)+1)) // Delay before repeating
#define SWAP_DELAY_BETWEEN_RETRIES		(unsigned long)(  25UL) // * (unsigned long) (((panstamp.cc1101.devAddress-1)%8)+1)) //
...
class SWPACKET : public CCPACKET
{
  ...
  public:
     ...
     *  False otherwise
     */
    boolean send(unsigned int, unsigned long, unsigned long);
};
